...
const NodeTree = memo(({ isCollapsed }: IProps) => {
  const { nodes, selectedPath, moveToNode, onExpandNode } =
    useContext(FlowContext);

  const mapNodesToNavLinks = useCallback(
    (
      ...
      let filteredNodes = nodes
        ? nodes.filter((node) => {
            return (
              includeCollapsed ||
              (parentId ? node.data.parentId === parentId : !node.data.parentId)
            );
          })
        : [];
      ...
      return filteredNodes.map((node) => {
        ...
        const childrenLinks = includeCollapsed
          ? undefined
          : mapNodesToNavLinks(node.id);
  ...
  const handleLinkExpandClick = useCallback(
