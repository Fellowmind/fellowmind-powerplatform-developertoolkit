...
export class IbanValidator implements ComponentFramework.StandardControl<IInputs, IOutputs> {
    ...
    private ibanAccountNumber: string;
    ...
    private onIbanChange(value: string) 
    {
		this.ibanAccountNumber = value;
		this.notifyOutputChanged();
	}

    public setIban(valid: boolean, iban: string) {
        if(!valid) {
            // @ts-ignore
			this.context.utils.setNotification("Iban is not valid", "iban-not-valid-123");
        } else {
             // @ts-ignore
            this.context.utils.clearNotification("iban-not-valid-123")
        }
    ...
    {
        return { ibanAccountNumber: this.ibanAccountNumber };
    }
