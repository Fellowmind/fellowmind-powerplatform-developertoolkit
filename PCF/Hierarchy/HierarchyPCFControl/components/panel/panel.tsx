...
const SidePanel = memo(() => {
  const { context, forms, activeForm, setActiveForm } =
  ...
  const menuIcon = useMemo(
    () => (isCollapsed ? "OpenPaneMirrored" : "OpenPane"),
    [isCollapsed]
  );

  console.log("isCollapsed: ", isCollapsed);
  const onFormChanged = useCallback(
    (_: React.FormEvent<HTMLDivElement>, item?: IDropdownOption): void => {
      if (!item) return;
  ...
  const formOptions = useMemo(
    () =>
      forms.map((f) => ({ key: f.formId, text: f.label } as IDropdownOption)),
    [forms]
  );
  ...
  return (
    ...
    >
      ...
      {/* <ActionButton
        ...
        iconProps={{
          iconName: direction !== "TB" ? "HorizontalTabKey" : "DistributeDown",
        }}
      >
        {isCollapsed
          ? ""
